...
def searchView(request):
    ...
    for t in allTrains:
        ...
        print(departing_station.pk , arriving_station.pk)
        if departing_station.pk < arriving_station.pk:
            print("1111111111111",t,date)
            try:
...
def bookView(request,chart,sourceSchedule,destSchedule,type,date):
    ...
    trainobj=Train.objects.get(name=train)
    trainno=trainobj.pk
    sourcetime=sourceSchedule.departure
    desttime=destSchedule.arrival
    sourceday=sourceSchedule.day
    destday=destSchedule.day
    trainstations=trainobj.train_schedule.all()
    stationss=[]
    for ts in trainstations:
        if ts.day:
            if destday==sourceday :
                if ts.day==sourceday and ts.departure>=sourcetime and ts.arrival<desttime:
                    stationss.append(ts)
            else :
                if ts.day==sourceday:
                    if ts.departure>=sourcetime :
                        stationss.append(ts)
                else :
                    if ts.arrival < desttime:
                        stationss.append(ts)
    print(stationss)
    print(trainno)
...
def complexBookView(request,chart1,chart2,sourceSchedule,commonSchedule1,commonSchedule2,destSchedule,type,date):
    ...
    trainobj=Train.objects.get(name=train1)
    trainno=trainobj.pk
    sourcetime=sourceSchedule.departure
    desttime=commonSchedule1.arrival
    sourceday=sourceSchedule.day
    destday=commonSchedule1.day
    trainstations=trainobj.train_schedule.all()
    stations=[]
    for ts in trainstations:
        if ts.day:
...
def confirmTicketView(request,chart,sourceSchedule,destSchedule,type,date):
    ...
    for i in range(seats):
        ...
        board=request.POST.get("board"+str(i))
        boardSchedule=Schedule.objects.get(id=board)
        board = boardSchedule.station_id
        board=Station.objects.get(pk=board)
        b=Ticket()
        b.passenger=name
        b.train=train
        b.type=type
        b.chart=chart
        b.board_schedule = boardSchedule
        b.user=user
        b.board=board
        b.source=source
        b.dest=dest
...
def complexConfirmTicketView(request,chart1,chart2,sourceSchedule,commonSchedule1,commonSchedule2,destSchedule,type,date):
    ...
    commonSchedule1 = Schedule.objects.get(pk=commonSchedule1)
    commonSchedule2 = Schedule.objects.get(pk=commonSchedule2)
    intermediate=commonSchedule2.station
    destSchedule = Schedule.objects.get(pk=destSchedule)
    source = sourceSchedule.station
    dest = destSchedule.station
    user= request.user
    ...
    for i in range(seats):
        ...
        b.chart = chart2
        b.user = user
        b.source = intermediate
        b.dest = dest
        b.source_schedule = commonSchedule2
        b.board_schedule = commonSchedule2
        b.dest_schedule = destSchedule
        b.date = date
        b.board=intermediate
        b.calculateFare()
        b.save()
